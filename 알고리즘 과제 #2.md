...
![1574332572497](https://user-images.githubusercontent.com/59441164/82116707-b7aaa280-97a6-11ea-8a92-2c1d553f3d73.png)

1. **batch mode **

   명령 프롬포트로 실행 시 batch 파일명을 parameter(argv[1])로 전달하면, 해당 파일명을 가진 파일을 열려고 시도한다. 파일명을 생략할 경우 해당 프로그램의 기본 파일명은 `batch.txt`로 입력되어 작동한다.

2.  **interactive mode**

   console에서 대기하며 하나씩 입력을 받으면서, 받은 입력에 대한 tree 구조를 출력하고, 해당 command 관련 정보를 출력한다.

   

### 3. 2-3-4 data structure 설명
...
교과서 알고리즘과 비교해서 insertion, deletion, searching 알고리즘은 동일하고, 더 효율적인 child pointer 관리를 위해서 static queue를 사용했다.
